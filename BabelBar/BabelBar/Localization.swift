...
enum Loc {
    ...
    private static let table: [LKey: [String]] = [
        ...
        .tipVoiceCommands: r(
            "Say «comma», «period», «colon», «dash», «new line», «paragraph» and the mark itself is inserted instead of the word. Works offline, in every dictation mode, with no delay. Only the plain dictionary form counts as a command, so a sentence that mentions punctuation stays as text.",
            "Скажи «запятая», «точка», «двоеточие», «тире», «с новой строки», «перенос строки», «абзац» — вместо слова подставится сам знак или перенос. Работает офлайн, во всех режимах диктовки и без задержки. Командой считается только слово в именительном падеже, поэтому фраза, где знак просто упоминается («поставь запятую»), останется текстом.",
